...
/* Find or add device */
static AppleDevice* find_or_add_device(AirDox* app, const uint8_t* mac, int8_t rssi) {
...
/* Parse Nearby packet */
static void parse_nearby(AirDox* app, const uint8_t* mac, int8_t rssi, const uint8_t* data, uint8_t len) {
    if(len < 2) return;
...
static void parse_airpods(AirDox* app, const uint8_t* mac, int8_t rssi, const uint8_t* data, uint8_t len) {
    if(len < 9) return;
...
static bool parse_apple_data(const uint8_t* mfg_data, uint8_t len, AirDox* app, const uint8_t* mac, int8_t rssi) {
...
/* Remove old devices */
static void remove_old_devices(AirDox* app) {
...
static void draw_cb(Canvas* canvas, void* ctx) {
    ...
    canvas_draw_str_aligned(canvas, 1, 2, AlignLeft, AlignTop, "AirDox");
